...
def plot_project_delays(results):
    """
    Create a visualization showing the comparison between target dates and actual completion
    times for each project, along with the associated penalties
    """
    num_projects = results['raw_data']['NUM_project']
    
    # Prepare data
    projects = [f'Project {j+1}' for j in range(num_projects)]
    targets = [results['raw_data']['Target'][j] for j in range(num_projects)]
    completions = [results['completion_times'][j] for j in range(num_projects)]
    delays = [results['delays'][j] for j in range(num_projects)]
    penalties = [results['penalties'][j] for j in range(num_projects)]
    
    # Create figure with two subplots - fixed size that won't exceed matplotlib limits
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
    
    # Plot 1: Target vs Actual completion times
    x = np.arange(len(projects))
    width = 0.35
    
    ax1.bar(x - width/2, targets, width, label='Target Date', color='#3498db', alpha=0.7)
    ax1.bar(x + width/2, completions, width, label='Actual Completion', color='#e74c3c', alpha=0.7)
    
    # Add labels and annotations
    for i, (target, completion) in enumerate(zip(targets, completions)):
        ax1.text(i - width/2, target + 5, f'{int(target)}', ha='center', va='bottom', color='#2c3e50')
        ax1.text(i + width/2, completion + 5, f'{int(completion)}', ha='center', va='bottom', color='#c0392b')
    
    ax1.set_ylabel('Time (days)')
    ax1.set_title('Target vs. Actual Completion Times', fontsize=14, fontweight='bold')
    ax1.set_xticks(x)
    ax1.set_xticklabels(projects)
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    # Plot 2: Delays and Penalties
    ax2_twin = ax2.twinx()
    
    bars1 = ax2.bar(x, delays, width, color='#f39c12', alpha=0.7, label='Delay (days)')
    bars2 = ax2_twin.bar(x + width, penalties, width, color='#9b59b6', alpha=0.7, label='Penalty (NIS)')
    ...
    for i, penalty in enumerate(penalties):
        offset = min(500, penalty * 0.1)  # Use 10% of penalty value but cap at 500
    ...
    ax2.set_title('Project Delays and Penalties', fontsize=14, fontweight='bold')
    ...
    # Lower DPI from 300 to 100 to reduce image size
    plt.savefig('project_delays.png', dpi=100, bbox_inches='tight')
    plt.close()
